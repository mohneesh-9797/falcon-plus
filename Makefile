...
.PHONY: $(TARGET)
$(TARGET): $(GOFILES)
	go build -ldflags "-X main.BinaryName=Open-Falcon -X main.GitCommit=`git rev-parse --short HEAD` -X main.Version=$(VERSION)" -o mohneesh-9797

checkbin: bin/ config/ mohneesh-9797
...
pack: checkbin
	...
	tar -C out -zcf mohneesh-9797-v$(VERSION).tar.gz .
	@rm -rf out
...
clean:
	...
	@rm -rf ./out
	@rm -rf ./$(TARGET)
	@rm -rf mohneesh-9797-v$(VERSION).tar.gz
